...
class HotterDeployer(object):
    def __init__(self, workspace_directory, tomcat_directory):
        ...
        def class_exclude_filter(path):
            return path.find('.svn') != -1 or path.find('target/classes') == -1
        self.wm.add_watch(workspace_directory, pyinotify.IN_CLOSE_WRITE,
                            rec=True, auto_add=True, proc_fun=OnClassChangedHandler(hotterDeployer=self),
                            exclude_filter=class_exclude_filter)
           ...
           if os.path.isdir(path) and file_name != '.svn' and file_name != 'target':
               ...
               if file_name == 'pom.xml':
                   ...
                   if not any(map(lambda x: portlet_name.endswith('-'+x), ['portlet', 'hook', 'theme', 'web'])):
                       portlet_name += '-'+getElementsByTagName(xmldoc, 'version')[0].firstChild.nodeValue
...
class OnFileChangedHandler(pyinotify.ProcessEvent):
    ...
    def _is_jsp_hook(self, cwd, rel_path):
    ...
    def process_IN_CLOSE_WRITE(self, event):
        ...
        if portlet_name:
            ...
            jsp_hook = self._is_jsp_hook(cwd, rel_path)
            if jsp_hook:
                print 'JSP HOOK', rel_path
                rel_path = jsp_hook
                latest_subdir = os.path.join(self.hotterDeployer.tomcat_directory, 'webapps', 'ROOT')
...
class OnClassChangedHandler(pyinotify.ProcessEvent):
    ...
    def process_IN_CLOSE_WRITE(self, event):
        cwd = event.pathname.split('/target/classes')[0]

        # Handle portlets
        portlet_name = self.hotterDeployer.portlets.get(cwd, None)
        print portlet_name
        if portlet_name:
            rel_path = event.pathname.split(cwd+'/target/classes')[1][1:]
            print rel_path
